...
describe('Database Schema', function() {
  ...
  describe('Encryption Keys', function() {
    ...
    it('should insert a key', function(done) {
      client.write("INSERT INTO encryption_keys (encrypted_key) VALUES ('') RETURNING *")
        .then(function(result) {
          ...
          encryption_key = UUID.validate(result.rows[0].uuid);
          expect(encryption_key).to.be.a('string');
          done();
        })
        .catch(done);
    });
    ...
    it('should retrieve our key', function(done) {
      if (! encryption_key) return this.skip();

      client.write("SELECT * FROM encryption_keys WHERE uuid = $1", encryption_key)
        .then(function(result) {
          expect(result.rows).to.have.length(1);
          expect(result.rows[0].uuid).to.equal(encryption_key);
          expect(result.rows[0].deleted_at).to.be.null;
          done();
        })
        .catch(done);
    ...
    it('should not allow modifications', function(done) {
      ...
      client.write("UPDATE encryption_keys SET encrypted_key='foo' WHERE uuid = $1", encryption_key)
        .then(function(result) {
          throw new Error('Should not work')
        }, function(error) {
          expect(error.cause.message).to.equal('Attempting to update values of encryption key "' + encryption_key + '"');
          done();
        })
        .catch(done);
    });
    ...
    it('should allow deletions', function(done) {
      ...
      client.write("DELETE FROM encryption_keys WHERE uuid = $1", encryption_key)
        .then(function(result) {
    ...
    it('should not undelete a softly deleted key', function(done) {
      ...
      client.write("UPDATE encryption_keys SET deleted_at=NULL WHERE uuid = $1", encryption_key)
        .then(function(result) {
          throw new Error('Should not work')
        }, function(error) {
          expect(error.cause.message).to.equal('Attempting to update deleted encryption key "' + encryption_key + '"');
          done();
        })
        .catch(done);
    });
  ...
  describe('Encrypted Object (Basics)', function() {
    ...
    it('should not insert an object without encrypted data', function(done) {
      client.write("INSERT INTO objects (kind, encryption_key) VALUES ('domain', $1) RETURNING *", encryption_key)
        .then(function(result) {
          throw new Error('Should not work')
        }, function(error) {
          expect(error.cause.message).to.match(/constraint/);
    ...
    it('should not insert an object without kind', function(done) {
      client.write("INSERT INTO objects (encryption_key, encrypted_data) VALUES ($1, '') RETURNING *", encryption_key)
        .then(function(result) {
          throw new Error('Should not work')
        }, function(error) {
          // TODO: message is from trigger
          done();
        })
        .catch(done);
    });
    ...
    it('should insert a domain', function(done) {
      ...
      client.write("INSERT INTO objects (kind, encryption_key, encrypted_data) VALUES ('domain', $1, '') RETURNING *", encryption_key)
        .then(function(result) {
          expect(result.rows).to.have.length(1);
          expect(result.rows[0].uuid).to.exist;
          expect(result.rows[0].kind).to.equal('domain');
          expect(result.rows[0].parent).to.equal(result.rows[0].uuid);
          expect(result.rows[0].encryption_key).to.equal(encryption_key);
    ...
    it('should perform soft deletions', function(done) {
      ...
      client.write("SELECT * FROM objects WHERE uuid = $1", domain)
        ...
        .then(function(result) {
          ...
          // Recreate new
          return client.write("INSERT INTO objects (kind, encryption_key, encrypted_data) VALUES ('domain', $1, '') RETURNING *", encryption_key)
        })
        .then(function(result) {
          expect(result.rows[0].uuid).to.exist;
          domain = UUID.validate(result.rows[0].uuid);
...
// TODO indexes here!!!!
  ...
  describe('Encrypted Object (Hieararchy)', function() {
    ...
    it('should insert a user for a domain', function(done) {
      ...
      client.write("INSERT INTO objects (kind, parent, encryption_key, encrypted_data) VALUES ('user', $1, $2, '') RETURNING *", domain, encryption_key)
        .then(function(result) {
          expect(result.rows).to.have.length(1);
          expect(result.rows[0].uuid).to.exist;
    ...
    it('should not insert credentials for a domain', function(done) {
      ...
      client.write("INSERT INTO objects (kind, parent, encryption_key, encrypted_data) VALUES ('credentials', $1, $2, '') RETURNING *", domain, encryption_key)
        .then(function(result) {
          throw new Error('Should not work')
        }, function(error) {
    ...
    it('should insert credentials for a user', function(done) {
      ...
      client.write("INSERT INTO objects (kind, parent, encryption_key, encrypted_data) VALUES ('credentials', $1, $2, '') RETURNING *", user, encryption_key)
        .then(function(result) {
          expect(result.rows).to.have.length(1);
          expect(result.rows[0].uuid).to.exist;
